...
test('Player emits a heartbeat immediately and every thirty seconds', async ({ page }) => {
    await page.clock.install({ time: new Date('2026-09-12T03:00:00.000Z') });
    let heartbeatRequests = 0;
    page.on('request', request => {
        if (request.method() === 'POST' && request.url().includes('/api/monitoring/heartbeat')) {
            heartbeatRequests += 1;
        }
    });

    await page.goto('/player?screen_id=controlled-clock-screen&token=demo-token');
