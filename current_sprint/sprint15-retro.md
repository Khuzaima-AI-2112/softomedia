...
```bash
# For any file a sprint story claims to CREATE:
find <path> -name "<filename>"   # If match found → reclassify as EDIT; read the file
# For any file a sprint story claims to EDIT:
cat <path>/<filename>            # Read in full before writing any spec instruction
```

### GUARDRAIL-10 — Inspect middleware chain of existing handlers before hardening *(NEW S15)*
When a sprint story proposes to harden an existing route (add auth, add role guard, add rate limit), the **full middleware chain of that handler must be read from source** before the spec is written. Checklist:
- [ ] Is the route currently public or authenticated?
- [ ] If authenticated, does it have a role guard?
- [ ] What other handlers in the same router share the same middleware?
- [ ] Would adding auth break any existing caller (frontend page, test, or external client)?

Failure to run this checklist is how SEC-S15-5 and SEC-S15-6 reached production as latent vulnerabilities.

### GUARDRAIL-11 — Enumerate downstream callers before hardening a public endpoint *(NEW S15)*
Before adding `authenticate` or `requireRole` to any route that is currently `public` or `authenticate`-only:
1. `grep -r "<route-path>" client-app/src --include="*.jsx" --include="*.js"` — find all frontend callers.
2. For each caller, confirm it passes a valid auth token in its request.
3. If any caller does not pass a token → **STOP**. Coordinate the frontend fix in the same PR or in a preceding commit. Do not land the backend hardening until the frontend is updated.

Skipping this check produces a "works in isolation" hardening that breaks the UI silently.

### GUARDRAIL-12 — Confirm service-layer pattern before creating a service file *(NEW S15)*
Before creating a new `*Service.js` file:
1. Check whether the domain's router already calls its repository directly.
2. If yes, a service layer is **optional** — document the decision explicitly and do not create the file by default.
3. If a service layer already exists for this domain, use it. Do not create a parallel one.

New service files that duplicate repository logic are spec debt, not architecture.

### GUARDRAIL-13 — Doc and nav stories are blocking, not best-effort *(NEW S15)*
`API_ROUTES.md`, `DATABASE_SCHEMA.md`, and sidebar nav link stories are **blocking items in the Definition of Done**. They are not post-sprint cleanup. The sprint is not closed until:
- Every new route has a row in `API_ROUTES.md`.
- Every new Firestore collection has a schema in `DATABASE_SCHEMA.md`.
- Every new page accessible only via direct URL has a sidebar nav link (or is explicitly documented as nav-less with a reason).

### GUARDRAIL-14 — Carry-over risks must have a resolution sprint assigned *(NEW S15)*
Any risk carried from a prior sprint (e.g., S11-1/2/4 persistence QA) must have one of:
- A scheduled resolution sprint (e.g., "targeted in S16"), OR
- An explicit deferral decision with a reason (e.g., "deferred post-MVP — Firestore emulator not available in CI")

A risk with no owner and no resolution sprint is a planning gap, not a risk register entry. If a risk has been carried for 3+ sprints with no action, it must be escalated to a decision: fix it, defer it explicitly, or close it as won't-fix.
