...
| 1.3 | `GET /api/users` with valid headers | HTTP **200**, array |
...
| # | What to do | Pass |
|---|---|---|
| 3.1 | `GET /api/loops` | Body has **`loops` array** + `business_hours` — NOT bare array |
| 3.2 | `GET /api/loops?screenid=1&date=2026-05-19` | `loops` filtered for screen 1 |
| 3.3 | `GET /api/loops?screen_id=1&date=2026-05-19` | **Same result as 3.2** |
...
| 4.6 | Click Play → Network shows `GET /api/loops?screenid=X&date=...` | ✓ |
| 4.7 | Loop list renders OR "no loops" message — no blank screen | ✓ |
...
| # | What to do | Pass |
|---|---|---|
| 5.1 | `demo_role = superadmin` → reload → Retailer Context selector **visible** | ✓ |
| 5.2 | `demo_role = retailer_admin` → reload → selector **gone** | ✓ |

---

## 🚨 Critical fail signals

- `GET /api/loops` returns bare `[]` → P0 fix reverted
